...
- **Forbidden Mistakes (re-read every turn before editing):** A pinned list of silent-failure rules. Violating any of these breaks the app or wastes hours. Copy this block verbatim into every plan:
  ...
  - **Code rules:**
    ...
    - No `import ... from "next/link"` or `from "next/navigation"` in app code. Always import from `@/lib/nav`. Halerium's nginx STRIPS the `/apps/<runner>/<port>/` prefix before forwarding to the server, so `next/link` would push the bare path to the URL bar — refresh and direct deep links would 404. The wrappers in `@/lib/nav` add the prefix to every `<Link>`, `router.push/replace/prefetch`, and strip it from `usePathname()`.
    - No `basePath` in `next.config.ts`. Use `assetPrefix: NEXT_PUBLIC_BASE_PATH`. With `basePath` set, the server requires the prefix on incoming requests — the proxy strips before forwarding, so every request 404s while logs look clean. Symptom: app starts fine, browser shows the 404 page, every JS/CSS file 404s in DevTools.
